...
# For every Dart file, delete only the line immediately following MARK:REMOVE
find . -type f -name "*.dart" -print0 | while IFS= read -r -d '' file; do
  if grep -q 'MARK:REMOVE' "$file"; then
    tmp_file="${file}.tmp.$$"
    awk '
      /MARK:REMOVE/ {
        print;   # keep the MARK:REMOVE line
        getline; # read and skip the next line
        next;    # continue with following lines
      }
      { print }
    ' "$file" > "$tmp_file"
    mv "$tmp_file" "$file"
